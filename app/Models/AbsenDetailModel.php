    ...
    public function riwayatBulan($id_pegawai, $month = NULL, $year = NULL)
{
    ...
    $riwayats = $riwayats
        ...
        ->groupBy('pegawai.id_pegawai, SUBSTRING(tgl_absen, 4, 3), SUBSTRING(tgl_absen, 8, 4)')
        ->select('COALESCE(COUNT(CASE WHEN detail_absensi.status_masuk IS NULL THEN 1 END), 0) as onTimeIn')
        ->select('COALESCE(COUNT(CASE WHEN detail_absensi.status_keluar IS NULL THEN 1 END), 0) as onTimeOut')
        ->select('COALESCE(SUM(CASE WHEN detail_absensi.absen_keluar != 0 THEN detail_absensi.absen_keluar - detail_absensi.absen_masuk ELSE 0 END), 0) as totalJamKerja')
        ->orderBy('detail_absensi.id_detail_absensi', 'ASC')
        ->get()
        ->getResultObject();
